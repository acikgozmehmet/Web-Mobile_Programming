...
public class EmployerActivity extends AppCompatActivity {
    ...
    private void delete(){
        ...
        try {
            Calendar calendar = Calendar.getInstance();
            calendar.setTime((new SimpleDateFormat("dd/mm/yyyy")).parse(binding.foundedEditText.getText().toString()));
            date = calendar.getTimeInMillis();
        } catch (Exception ex){
            ex.printStackTrace();
        }
        ...
        ContentValues values = new ContentValues();
        values.put(SampleDBContract.Employer.COLUMN_NAME, binding.nameEditText.getText().toString());
        String selection = SampleDBContract.Employer.COLUMN_NAME+ " like ? and " +
                           SampleDBContract.Employer.COLUMN_FOUNDED_DATE + " > ? and " +
                           SampleDBContract.Employer.COLUMN_DESCRIPTION + " like ? ";
    ...
    private void update(){
        ...
        ContentValues values = new ContentValues();
        values.put(SampleDBContract.Employer.COLUMN_DESCRIPTION, binding.descEditText.getText().toString());
        String selection = SampleDBContract.Employer.COLUMN_NAME+ " like ?" ;

        String [] selectionArgs = {"%" + name };
        long newRowId = database.update(SampleDBContract.Employer.TABLE_NAME, values, selection, selectionArgs);
